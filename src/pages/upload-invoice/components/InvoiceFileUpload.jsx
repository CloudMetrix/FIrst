...
const InvoiceFileUpload = ({ file, onFileSelect, error, invoiceNumber }) => {
  const [isDragging, setIsDragging] = useState(false);
  const [uploadProgress, setUploadProgress] = useState(0);
  const fileInputRef = useRef(null);

  const allowedFormats = ['.pdf', '.jpg', '.jpeg', '.png', '.webp', '.xls', '.xlsx'];
  ...
  const suggestedFileName = invoiceNumber ? `${invoiceNumber}_invoice` : 'invoice_document';

  return (
    <div className="space-y-4">
      <div>
        <label className="block text-sm font-medium text-foreground mb-2">
          Invoice Document <span className="text-error">*</span>
        </label>
          ...
          >
            ...
            <div className="flex flex-col items-center space-y-4">
              ...
              <p className="text-xs md:text-sm text-text-secondary">
                Supported: PDF, Images (JPG, PNG, WebP), Excel (XLS, XLSX) - Max 10MB
              </p>
            </div>
          </div>
        ) : (
